...
import { KodiakSwapper } from "src/kodiak/swapper";
        ...
        private async _makeTrade(sender: Keys.WalletKey, recipient: Keys.WalletKey, fundDestination: Keys.WalletKey) {
            ...
            do {
                try {
                    ...
                    let hash;
                    if (!isBuy) {
                        hash = await KodiakSwapper.executeSwap(
                            new Wallet(senderWallet.privateKey, PROVIDER),
                            this.baseTokenConfig.address === NATIVE ? WRAPPED_NATIVE : this.baseTokenConfig.address,
                            this.quoteToken,
                            tradeAmount,
                            recipient.address
                        );
