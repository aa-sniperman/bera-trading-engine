...
const SWAP_ROUTER_GATEWAY_DNS = "https://swap.hold.so/bartio/api";

// Mapping of chain IDs to swap router contract addresses
const SWAP_ROUTER_ADDRESSES = "0x123C070bFC0267Ea7905CFD691dDC78963443D86";

// Address placeholder for the native token
...
function isNativeToken(contractAddress: string) {
  if (contractAddress.toLowerCase() === NATIVE_ADDRESS.toLowerCase()) {
    return true;
  }
...
async function sendTransaction({
  ...
  populatedTx.gasLimit = await wallet.estimateGas(populatedTx);

  populatedTx.nonce = await wallet.getTransactionCount();

  return await wallet.sendTransaction(populatedTx);
}

// ================================================================================================
// Main func

// example curl swap 1 Bera-USDC
export async function SwapHoldSo({
  ...
  selectedAddress, // wallet swap from private key.
  ...
  const valueBatchTx = isNativeToken(from) ? amountIn : "0";

  const populatedTx = {
    data: data.tx.data,
    from: selectedAddress,
    to: SWAP_ROUTER_ADDRESSES,
    value: ethers.BigNumber.from(valueBatchTx),
  };
